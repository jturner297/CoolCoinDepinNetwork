...
1. Validator Setup:
- Install dependencies: pip install fastapi uvicorn pydantic cryptography requests python-multipart
- Run server: uvicorn server:app --reload --host 0.0.0.0 --port 8000
- Run Ipconfig in terminal to find the validator IP

2. Miner Setup:
- Connect BME280 to Pi (I2C pins SCL/SDA)
- Install dependencies: pip install adafruit-circuitpython-bme280 requests cryptography
- Generate the miner's wallet:
	- private key: openssl genpkey -algorithm Ed25519 -out miner_wallet.pem
	- public key: openssl pkey -in miner_wallet.pem -pubout -out miner_wallet.pub
- Update server_url in cool_coin_miner.py to validator IP
- Run miner: python cool_coin_miner.py
- Miner submits MINT transactions every second

3. Dashboard and Wallet Client:
- Open cool_coin_dashboard.HTML in browser
- Update fetch calls to validator IP
- Generate Wallet 2: python wallet_generation.py
- Test transfer: python transfer.py (Wallet 1 -> Wallet 2)
